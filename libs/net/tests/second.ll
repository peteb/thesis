...
@__sym_client = weak global i32 0                 ; <i32*> [#uses=4]
...
entry:
  ...
  %client = alloca i8*                            ; <i8**> [#uses=3]
  store i8* %retval4, i8** %client
  %11 = call i8* @closure_object_place()          ; <i8*> [#uses=4]
  ...
  %15 = load i8** %client                         ; <i8*> [#uses=1]
  %16 = call i8* @__t_clone(i8* %15)              ; <i8*> [#uses=1]
  %17 = call i8* @object_set_slot(i8* %11, i32* @__sym_client, i8* %16, i16 0) ; <i8*> [#uses=0]
  %18 = call i8* @closure_set_fptrC(i8* %11, %fptr @__closure1) ; <i8*> [#uses=0]
  %hej = alloca i8*                               ; <i8**> [#uses=2]
  store i8* %11, i8** %hej
  %19 = load i8** %hej                            ; <i8*> [#uses=1]
  %20 = call i8* @array_object(i32 0)             ; <i8*> [#uses=1]
  %calltmp5 = call %ret @object_exec(i8* %19, i32* @__sym_apply, i8* %20) ; <%ret> [#uses=1]
  %retval6 = extractvalue %ret %calltmp5, 0       ; <i8*> [#uses=1]
  %21 = load %ret* %__retval                      ; <%ret> [#uses=1]
  %22 = insertvalue %ret %21, i8* %retval6, 0     ; <%ret> [#uses=1]
  store %ret %22, %ret* %__retval
  br label %ret

ret:                                              ; preds = %entry
  %23 = load %ret* %__retval                      ; <%ret> [#uses=1]
  ret %ret %23
}
...
entry:
  %__retval = alloca %ret                         ; <%ret*> [#uses=3]
  %3 = call i8* @object_get_slot(i8* %1, i32* @__sym_client) ; <i8*> [#uses=0]
  %4 = call i8* @object_get_slot(i8* %1, i32* @__sym_client) ; <i8*> [#uses=1]
  %5 = load %ret* %__retval                       ; <%ret> [#uses=1]
  %6 = insertvalue %ret %5, i8* %4, 0             ; <%ret> [#uses=1]
  store %ret %6, %ret* %__retval
  br label %ret

ret:                                              ; preds = %entry
  %7 = load %ret* %__retval                       ; <%ret> [#uses=1]
  ret %ret %7
}
