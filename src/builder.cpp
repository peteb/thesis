...
llvm::Value * Builder::CreateStoreReturnMRV(llvm::Value * ret, llvm::Value * exc) {
  ...
  Type * objectTy = Builder::getObjectType(_mod);
...
llvm::Value * Builder::CreateSymbol(const std::string & name) {
  ...
  std::string symName = "__sym_" + fixedName;
  Type * rettype = Type::getInt32Ty(_mod->getContext());
...
llvm::Value * Builder::CreatePtr(object_t ptr) {
  Type * objectTy = getObjectType(_mod);
...
llvm::Type * Builder::getObjectType(llvm::Module * module) {
  return Type::getInt8PtrTy(module->getContext());
}
